<script lang="ts">
  ...
  function randomizeQuiz() {
    ...
    shuffle(questions)
    validateImage()
    firstTen = questions.slice(0,10)
  }

  // Process Images
  function validateImage(){
    questionImages = new Array(10).fill(null);
    firstTen?.forEach(function(q,i){
      if(q.questionImage){
  ...
  $: progressPercentage = questionsAnswered * 10;
...
<section>
  {#if questionPointer == -1}
    <div class="start-screen">
      <p>Quiz yourself on what you just learned!
            ...
            <div class="quiz-screen">
              <div class="main">
                <div class="card">
                  ...
                  >
                    <div class="progress-bar overflow-visible progress-bar-striped progress-bar-animated bg-success" style="width: {progressPercentage}%; text-align: center;"></div>
                  </div>
                  <h4 class="card-header">{firstTen[questionPointer].question}</h4>
                  {#if questionImages[questionPointer]}
                    <img 
                      class="img-fluid"
                      src={urlFor(questionImages[questionPointer]).url()}
                      alt=""
                    >
                  {/if}
                </div>
                <div class="answerOption">
                  {#each firstTen[questionPointer].answerOption as opt,i}
                    <div class="d-grid gap-2 col-12 mx-auto">
                      <button 
                        type="button"
                        class="btn active m-2" 
                        on:click={() => {answers[questionPointer]=i;}}> 
                        <span>
                          {#if answers[questionPointer]==i}
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi-square-fill" viewBox="0 0 16 16">
              ...
              <div class="quiz-button">
                <button 
                  ...
                  on:click={() => questionPointer === 0 ? questionPointer = questionsLength - 1 : questionPointer--}>
                </button>
                <button 
                  class="btn btn-secondary mt-5 mbri-arrow-next" 
                  on:click={() => questionPointer + 1 == questionsLength ? questionPointer = 0 : questionPointer++}>
                </button>
              </div>
              <div>
                <button 
                class="btn btn-primary mt-5" 
                title="answer all 10 questions to submit" 
                    ...
                    <div class="collapse" id="collapseExample{i}">
                      <div class="card card-body">
                        <ul class="list-group list-group-flush">
                          <li class="list-group-item"><span class="text-start fw-bolder">Your Answer:</span>{result.userAnswer}</li>
                          {#if !result.maru}
                            <li class="list-group-item"><span class="text-start fw-bolder">Correct Answer:</span>{result.correctAnswer}</li>
                          {/if}
                          {#if result.note}
                            <li class="list-group-item"><span class="text-start fw-bolder">Note:</span>{result.note}</li>
                          {/if}
                        </ul>
                      </div>
                    </div>
